...
## 目标
这份文档定义 OpenHamster 进入 `v2` 后，未来 `2-4` 周应该持续跟踪什么，而不是继续盲目扩功能。
...
## 6. 多标的并行研究（启动节奏）
目标：在不破坏当前单标的稳定性的前提下，逐步扩展到 `multi-symbol parallel research（多标的并行研究）`。

建议启动时间（按当前日期 `2026-03-19`）：
1. `2026-03-23` 当周：进入设计与影子评估阶段，只做只读对照，不改当前入池逻辑。
2. `2026-03-30` 当周：开启小规模并行研究（建议 `3` 只标的），仍维持 `1` 个模拟盘席位。
3. `2026-04-06` 当周：若运行稳定，再扩到 `5-8` 只研究标的，并评估是否引入有限多席位模拟盘。

启动前置条件：
- runtime 连续稳定（无持续卡 stage、无频繁失败）
- 单标的流程连续产出可解释 evidence（证据）
- provider fallback 率可控，readiness 不出现异常跳变

阶段验收：
- 并行研究不影响现有 pipeline 周期
- 候选池质量分布没有明显恶化
- 审计台可解释每个 symbol 的入池与淘汰因果

## 7. Agent 持续学习量化知识库（启动节奏）
目标：让 `strategy knowledge layer（策略知识层）` 持续吸收“系统自己跑出来的证据”，而不是静态目录。

建议启动时间（按当前日期 `2026-03-19`）：
1. `2026-03-23` 当周：先上线“知识命中统计”只读报表（各家族命中率、失败模式命中率、新颖性分布）。
2. `2026-03-30` 当周：上线“每周知识复盘”流程，自动生成候选知识修订建议（不自动生效）。
3. `2026-04-13` 当周：上线“人工审批后生效”的知识更新流程，写入审计事件。

固定原则：
- 学习来源优先内部证据：proposal、risk decision、backtest、paper execution
- 所有知识修订都要审计留痕，且不得覆写历史判断上下文
- 新知识先观察再放权，不直接改变 risk gate 的硬阈值
