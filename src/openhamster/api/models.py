...
class EventRecord(Base):
    ...
    __table_args__ = (
        Index("ix_event_records_market_published_at", "market_scope", "published_at"),
    )
...
class DailyEventDigest(Base):
    ...
    __table_args__ = (
        UniqueConstraint("trade_date", "market_scope", "symbol_scope", name="uq_daily_digest_scope"),
        Index("ix_daily_event_digests_scope_trade_date", "market_scope", "symbol_scope", "trade_date"),
    )
...
class AuditRecord(Base):
    ...
    config_version: Mapped[str] = mapped_column(String(64), nullable=False, default="v0.2.0")
    payload: Mapped[dict] = mapped_column(JSON, nullable=False, default=dict)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)

    __table_args__ = (
        Index("ix_audit_records_event_created_at", "event_type", "created_at"),
        Index("ix_audit_records_entity_event_created_at", "entity_type", "event_type", "created_at"),
        Index("ix_audit_records_decision_created_at", "decision_id", "created_at"),
    )
