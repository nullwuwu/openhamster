...
class BacktestEngine:
    ...
    支持自定义数据源:
        from goby_shrimp.data import TwelveDataProvider
    ...
    def _create_default_provider(self) -> DataProvider:
        """创建默认数据源（优先 settings.data_source.provider）"""
        settings = get_settings()
        configured_provider = settings.data_source.provider
